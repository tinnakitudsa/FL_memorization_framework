# thank for the codew: https://github.com/Brit7777/LM-plagiarism?tab=readme-ov-file
...
def frag_founder(
    ps, src_offsets, susp_offsets, src_gap, susp_gap, src_size, susp_size, side
):
    """
...
def clustering(
    ps, src_offsets, susp_offsets, src_gap, susp_gap, src_size, susp_size, side, times
):
    """
...
def validation(
    ...
    while i < range_i:
        ...
        if sim_frag <= th3:
            ...
            if (
                src_gap > src_gap_least and susp_gap > susp_gap_least
            ):  # Do until substraction +1
                new_psr = clustering(
                    psr[i],
                    src_offsets,
                    susp_offsets,
                ...
                for ps_set in new_psr:
                    new_plags.append(
                        [
                            (min([x[0] for x in ps_set]), max([x[0] for x in ps_set])),
...
def remove_small_plags(plags, psr, src_offsets, susp_offsets, th):
    ...
    for i in range(len(plags)):
        arg1 = (
            susp_offsets[plags[i][0][0]][0],
            susp_offsets[plags[i][0][1]][0] + susp_offsets[plags[i][0][1]][1],
        )
        arg2 = (
            src_offsets[plags[i][1][0]][0],
            src_offsets[plags[i][1][1]][0] + src_offsets[plags[i][1][1]][1],
        )
...
def word_span_tokenizer(text):
    ...
    for span in sent_detector.span_tokenize(text):
        ...
        for word in sent_words:
            ...
            # print pos
            offsets.append(
                [span[0] + idx + pos, idx + span[0] + pos + len(word)]
...
def longest_common_substring_all(
    s1, s1_off, s2, s2_off, th
...
def verbatim_det_lcs_all(
    plags, psr, susp_text, src_text, susp_offsets, src_offsets, th_shortest
):
    """
    ...
    while i < len(plags):  # For each plagiarism case
        ...
        res2 = common_substring_pro_all(
            susp_text[
                susp_offsets[plags[i][0][0]][0] : susp_offsets[plags[i][0][1]][0]
                + susp_offsets[plags[i][0][1]][1]
            ],
            src_text[
                src_offsets[plags[i][1][0]][0] : src_offsets[plags[i][1][1]][0]
            ...
            if type_plag == 1:
                ...
                for sub_case in res:
                    res_plags.append(
                        [
                            (
                                susp_offsets[plags[i][0][0]][0] + sub_case[0],
                                susp_offsets[plags[i][0][0]][0] + sub_case[1],
                            ),
                            (
                                src_offsets[plags[i][1][0]][0] + sub_case[2],
                                src_offsets[plags[i][1][0]][0] + sub_case[3],
                            ),
                        ]
                    )
                    res_psr.append(psr[i])
...
#############################################################
def safe_stem(stemmer, word):
    try:
        return stemmer.stem(word)  # Return as-is if not valid
    except:
        return word
        ...
        for word in word_detector.tokenize(sents[-1]):  # for each word in the sentence
            if word[0].isalnum() and len(word) > 2:
                if word not in stopwords:
                    word_pp = safe_stem(stemmer, word)
                    # word_pp = stemmer.stem(word) #Modify this to get above
                    # word_pp = stemmer.stemWord(word)# if word not in stopwords_all else word###Highest time consuming
                else:
                    word_pp = word
            else:
                continue
            if word_pp in sent_dic.keys():
                sent_dic[word_pp] += 1
            else:
                sent_dic[word_pp] = 1
                if word_pp in voc.keys():
                    voc[word_pp] += 1
                else:
                    voc[word_pp] = 1
        if len(sent_dic) > 0:
...
class SGSPLAG:
    ...
    def preprocess(self):
        ...
        """
        self.src_bow = tokenize(
            self.src_text, self.src_voc, self.src_offsets, self.src_sents, self.rem_sw
        )
        ss_treat(
            self.src_bow, self.src_offsets, self.min_sentlen, self.rssent, self.src_voc
        )
        self.susp_bow = tokenize(
            self.susp_text,
            self.susp_voc,
            self.susp_offsets,
            self.susp_sents,
    ...
    def compare(self):
        ...
        if type_plag == 0:
            ...
            if self.summary != 0:
                for plag in plags2:
                    arg1 = (
                        self.susp_offsets[plag[0][0]][0],
                        self.susp_offsets[plag[0][1]][0]
                        + self.susp_offsets[plag[0][1]][1],
                    )
                    arg2 = (
                        self.src_offsets[plag[1][0]][0],
                        self.src_offsets[plag[1][1]][0]
                        + self.src_offsets[plag[1][1]][1],
                    )
    ...
    for i in range(0, len(p), 2):
        if p[i] in p_list.keys():
            ...
            if type(p[i + 1]).__name__ == "str":
                ...
                print(
                    p[i], "must be", p_list[p[i]], "instead of", type(p[i + 1]).__name__
                )
                exit()
        else:
...
# OUR IMPROVEMENT : paraphrase detection
def identify_paraphrase(sent1, sent2, tokenizer, model):

    if sent1.lower().strip() == sent2.lower().strip():
        return False

    # sequence_0 = "is one of many that have been struggling with what it means when your reservation serves as the official venue."
    # sequence_1 = 'The tourism economy has been heavily hit by the coronavirus (COVID-19) pandemic, and measures introduced to contain its spread.'

    # sequence_0 = "The company HuggingFace is based in New York City. This question is linked closely to the debated issue of the pointed styles' beginnings."
    # sequence_1 = "This query is intimately connected with the disputed point of the origin of the pointed style itself. HuggingFace's headquarters are situated in Manhattan."

    # paraphrase = tokenizer.encode_plus(
    #     sent1, sent2, return_tensors="pt", truncation=True
    # )
    # model.eval()
    # with torch.no_grad():
    ...
    paraphrase_results = torch.softmax(
        paraphrase_classification_logits, dim=1
    ).cpu().tolist()[0]  # Move results to CPU before converting to list

    # print(paraphrase_results)
    return paraphrase_results[1]


# OUR IMPROVEMENT : NER recognition
def get_entities(txt1, txt2):
    ...
    for ent in doc1.ents:
        ...
        dict1.append({"text": ent.text, "label": ent.label_})
        entities1.add(ent.text)
    for ent in doc2.ents:
        # print(ent.text, ent.start_char, ent.end_char, ent.label_)
        dict2.append({"text": ent.text, "label": ent.label_})
        entities2.add(ent.text)
    ...
    if (
        entities1.intersection(entities2) == entities1
        or entities1.intersection(entities2) == entities2
    ):
        # print(txt1)
        # print(txt2)
        # print(entities1)
        # print(entities2)
        # print(entities1.intersection(entities2))
        # print("yes")
        return 1
    else:
        return 0
...
def serialize_features(
        ...
        for f in features:
            ...
            if type_plag:
                dic = {
                    "susp_id": susp,
                    "p_type": "verbatim",
                    "string": susp_text[f[0][0] : f[0][1]],
                    "susp_full_string": susp_text,
                    "src_id": src,
                }
                dics.append(dic)
                # count_verbatim += 1
            # summary
                ...
                dic = {
                    "susp_id": susp,
                    "p_type": "summary",
                    "susp_string": susp_text[f[0][0] : f[0][1]],
                    "susp_full_string": susp_text,
                    "src_id": src,
                    "source_string": source_text[f[1][0] : f[1][1]],
                }
                # count_summary += 1
                dics.append(dic)
            else:
                dic = {
                    "susp_id": susp,
                    "p_type": "random or translation",
                    "susp_string": susp_text[f[0][0] : f[0][1]],
                    "susp_full_string": susp_text,
                    "src_id": src,
                    "source_string": source_text[f[1][0] : f[1][1]],
                }
                comp1 = sent_tokenize(source_text[f[1][0] : f[1][1]])
                comp2 = sent_tokenize(susp_text[f[0][0] : f[0][1]])

                comp1 = [i for i in comp1 if i != "\n"]
                comp2 = [i for i in comp2 if i != "\n"]

                # # check if sentences are paraphrase
                # for i in comp1:
                #     for j in comp2:
                #         score = identify_paraphrase(i, j, tokenizer, model)
                #         if score > 0.5:
                #             para_pair_high.append([i, j, score])

                # # check NER
                # ner_result = 0

                # for pair in para_pair_high:
                #     ner_result = get_entities(pair[0].lower(), pair[1].lower())
                #     if ner_result and pair[2] < 0.99:
                #         dic["matching_ner"] = [pair[0], pair[1], pair[2]]
                #         break

                # # random_or_translate += 1
                # if len(para_pair_high):
                #     dic["para_pair_high"] = para_pair_high
                #     # dic = {'susp_id': susp, 'p_type' : 'random or translation', 'susp_string':susp_text[f[0][0]:f[0][1]], 'src_id': src, 'source_string': source_text[f[1][0]: f[1][1]], 'para_pair':para_pair}
                #     # dics.append(dic)
                ...
                #         score = identify_paraphrase(i, j, tokenizer, model)
                #         if score < 0.5:
                #             para_pair_low.append([i, j, score])

                # # check NER
                # ner_result = 0

                # for pair in para_pair_low:
                #     ner_result = get_entities(pair[0].lower(), pair[1].lower())
                #     if ner_result and pair[2] < 0.99:
                #         dic["matching_ner"] = [pair[0], pair[1], pair[2]]
                #         break
                ...
                # # random_or_translate += 1
                # if len(para_pair_low):
                #     dic["para_pair_low"] = para_pair_low
                #     # dic = {'susp_id': susp, 'p_type' : 'random or translation', 'susp_string':susp_text[f[0][0]:f[0][1]], 'src_id': src, 'source_string': source_text[f[1][0]: f[1][1]], 'para_pair':para_pair}
                #     # dics.append(dic)
                ...
                for pair in para_pair_high:
                    ner_result = get_entities(pair[0].lower(), pair[1].lower())
                    if ner_result:
                        dic["matching_ner_high"] = [pair[0], pair[1], pair[2]]
                        break

                # random_or_translate += 1
                if len(para_pair_high) and "matching_ner_high" in dic:
                    dic["para_pair_high"] = para_pair_high
                    # dic = {'susp_id': susp, 'p_type' : 'random or translation', 'susp_string':susp_text[f[0][0]:f[0][1]], 'src_id': src, 'source_string': source_text[f[1][0]: f[1][1]], 'para_pair':para_pair}
                    # dics.append(dic)
                ...
                for pair in para_pair_low:
                    ner_result = get_entities(pair[0].lower(), pair[1].lower())
                    if ner_result:
                        dic["matching_ner_low"] = [pair[0], pair[1], pair[2]]
                        break

                # random_or_translate += 1
                if len(para_pair_low) and "matching_ner_low" in dic:
                    dic["para_pair_low"] = para_pair_low
                    # dic = {'susp_id': susp, 'p_type' : 'random or translation', 'susp_string':susp_text[f[0][0]:f[0][1]], 'src_id': src, 'source_string': source_text[f[1][0]: f[1][1]], 'para_pair':para_pair}
                    # dics.append(dic)

                # add to dics
                if "matching_ner_low" in dic or "matching_ner_high" in dic:
                    dics.append(dic)

        # write txt file
        if len(dics):
            # outdir="/ist/users/tinnakitu/t5_storage/t5/aaaaaaa"
            # save_path = os.path.join(outdir, str(src) + "-" + str(susp) + ".json")
    ...
    # print(susp,src,features,type_plag,summary_flag,source_text,susp_text)
    return None




def memorization_evaluate(
    es,
    model,
    ...
    print("Counting memorization")

    model.to(device)
    model.eval()

    # Connect to Elasticsearch with authentication and certificate verification
    memorization_analysis = []

    count_non_serialized_pair = 0
    for i, tuple_prefix_suffix in tqdm(
        enumerate(generated_suffixes_processed),
        total=len(generated_suffixes_processed),  # Set the total for tqdm
        desc="Counting memorization",
    ):
        prefix = tuple_prefix_suffix["prefix"]
        suffix = tuple_prefix_suffix["suffix"]
        generated_suffix = tuple_prefix_suffix["generated_suffix"]

        # preprocess search queries
        stop_words = set(stopwords.words("english"))
        word_tokens = nltk.word_tokenize(generated_suffix)
        ...
        # for index parameter, set the index name you created on elasticsearch
        res = es.search(
            index=INDEX,
            size=top_k_match,
            query={"match": {"text": filtered_sentence}},
            request_timeout=300,
        )


        if len([hit for hit in res["hits"]["hits"]]) > 0:
            elastic_results = [hit for hit in res["hits"]["hits"]]
        else:
            continue

        for elastic_result in elastic_results:
            sgsplag_obj = SGSPLAG(
                elastic_result["_source"]["text"], generated_suffix, parameters
            )
            ...
            result = serialize_features(
                elastic_result["_id"],
                i,
                sgsplag_obj.detections,
                type_plag,
                summary_flag,
                generated_suffix,
                elastic_result["_source"]["text"],
                tokenizer,
                model,
            )
            ...
            if result:
                ...
                memorization_analysis.append(result_chunk)

    return memorization_analysis
